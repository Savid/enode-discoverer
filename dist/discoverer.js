...
import { BOOT_NODES, DNS_ADDRESS, DNS_NETWORKS, MAX_PEERS, REFRESH_INTERVAL, REMOTE_URL, REMOTE_SECRET, REMOTE_SEND_INTERVAL, } from '#app/constants';
import Metrics from '#app/metrics';
export default class Discoverer {
    static init() {
        if (!BOOT_NODES)
            throw new Error('no valid bootnodes');
        if (!DNS_NETWORKS)
            throw new Error('no valid dns networks');
...
Discoverer.bootnodes = [];
Discoverer.dnsNetworks = [];
Discoverer.discoveredEnodes = [];
